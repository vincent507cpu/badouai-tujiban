...
def load_data(path):
    ...
    with open(path, encoding="utf8") as f:
        for line in f:
            ...
            labels.append(tag)
    return corpus, all_words, all_class, labels
...
if __name__ == "__main__":
    ...
    corpus, all_words, all_class, _ = load_data(path)
    # print(corpus)
    # print(all_words)
    # print(all_class)
    ...
    with open(path, encoding="utf8") as f:
        for line in f:
            ...
            tag = line["tag"]
            query = line["title"]
            label = bayes_predict(query, bayes_feature_dict, all_class, 1)
            num += 1
            if label == tag:
                correct += 1
    # bayes_predict(query, bayes_feature_dict, all_class, topk=1)

    print(f'测试集中一共有 {num} 条样本，正确率为 {correct / num * 100:.2f} %。')
